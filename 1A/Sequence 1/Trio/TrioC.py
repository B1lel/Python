...
def tour(numero_joueur, nombre_joueur, Jeu, Score, humain):
    ...
    if humain :
        ...
        print("Voici sa main:", Jeu[numero_joueur])
       
    while cartes_identique :
        
        carte_choisi_result = carte_choisi(nombre_joueur, Jeu, humain)
        Carte_selected.append(carte_choisi_result)
        print(Carte_selected)
...
def carte_choisi(nombre_joueur, Jeu, humain):
    if humain :
        stop = False
        while not stop  : 
        ...
        numero_jeu_choisi = random.randint(0,len(Jeu) - 1)  
        print("Le bot a choisii le jeu",numero_jeu_choisi )              

    carte_choisi = choix_carte(numero_jeu_choisi, Jeu, humain) # {"valeur carte", "index jeu ", "index carte dans jeu"}
    
    return carte_choisi

def choix_carte(numero_jeu_choisi, Jeu, humain):  
    if numero_jeu_choisi == 0 :
        return choix_carte_centre(Jeu, humain)
    else:
        return choix_carte_joueur(numero_jeu_choisi, Jeu, humain)

def choix_carte_centre(Jeu, humain) :
    ...
    if humain :
        ...
        boucle = True
        while boucle :
            index_max = len(Centre)
            index_carte = input("Entrez la position de la carte que vous souhaitez révéler :"  )
            
            if not index_carte.isdigit()  :
                print(f"Veuillez entrez une valeur valide (1 à {index_max + 1}) ")
                continue
                 
            index_carte = int(index_carte) - 1
            
            if index_carte < 0 or index_carte > index_max :
                print(f"Veuillez entrez une valeur valide (1 à {index_max + 1}) ")
                continue
        ...
        while True :
            ...
            while not stop :
                ...
                index_max = len(Centre)
                index_carte = random.randint(0, index_max - 1 )
                #print("le bot a pris la carte au centre a la place",index_carte )
                
                if Centre[index_carte] == None :
                    print("Cette carte à déjà était tirée robot")
...
def choix_carte_joueur(numero_jeu_choisi, Jeu, humain) :
    if humain : 
        ...
        choix = random.choice([max_carte, min_carte])
        #print("le bot a choisi le ", choix )
        
        if choix == max_carte:
            #print("le bot a choisi le max", choix )
            carte_choisi = max_carte    
            index_carte = jeu_choisi.index(max_carte)
            jeu_choisi.remove(max_carte)
            Jeu.insert(numero_jeu_choisi,jeu_choisi)
            ...
            #print("le bot a choisi le min", choix )
            carte_choisi = min_carte
            index_carte = jeu_choisi.index(min_carte)
            jeu_choisi.remove(min_carte)
            Jeu.insert(numero_jeu_choisi,jeu_choisi)
...
def redistribution(Carte_selected, Jeu):
...
def check_humain(numero_joueur, numero_humain):
    if numero_joueur == numero_humain :
        return True 
    else :
        return False 
...
def main():
    ...
    victoire = False
    premier_tour = True
    numero_humain = random.randint(1,len(Jeu)-1)
    print("humain = joueur ", numero_humain)
    ...
    while not victoire:
        ...
        if premier_tour:
            ...
            humain = check_humain(numero_joueur, numero_humain)
            print(humain)
            victoire = tour(numero_joueur, nombre_joueur, Jeu, Score, humain)
